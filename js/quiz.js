...
function controleerAntwoord(gekozenIndex) {
    ...
    updateScoreboard(punten, streakCount);
}
...
function updateScoreboard(punten, streak) {
    ...
    if (scoreElement) {
        ...
        setTimeout(() => scoreElement.classList.remove('animate__animated', 'animate__pulse'), 500);
    }
    ...
    if (bonusElement) {
        const bonus = punten - 10; // Basis punten zijn 10
        bonusElement.textContent = bonus > 0 ? `+${bonus}` : '0';
    }

    // Update multiplayer score als we in een game zitten
    if (gameState.teamCode) {
        const gameRef = firebase.database().ref(`games/${gameState.teamCode}/players/${gameState.playerName}`);
        gameRef.set(score);
    }
} 